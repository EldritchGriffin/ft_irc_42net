...
void Server::init_server()
{
    ...
    {//TODO learn why we cast (struct sockaddr*)sockaddr_in in bind();
...
void Server::accept_client()
{
    ...
    while (1)
    {
        poll_flag = poll(fds, 1, 0);
        if (poll_flag < 0) {std::cout << "zebi" << std::endl; exit(0);}

        if(poll_flag > 0)
        {
            if (fds[0].revents == POLLIN)
            {
                read_size = recv(client_socket, client_name, message_buffer, 0); // it fill the 2nd parameter with the length of the 3rd parameter from the socket specified in the 1st parameter ; the last parameter is the flags seems like open()
                if(read_size == -1)
                {
                ...
                send(client_socket, "well recieved\n", strlen("well recieved\n"), 0);
            }
        }
    }
...
void Server::run()
{
    ...
    //FIXME make the server hang waiting for any client to join or send requests to the socket;
    accept_client();
}